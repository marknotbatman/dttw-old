...
function like_scripts() {
	wp_enqueue_script( 'jm_like_post', SKYLINE_THEME_URI .'/js/post-like.min.js', array('jquery'), '1.0', 1 );
...
function getPostLikeLink( $post_id ) {
	...
	if ( AlreadyLiked( $post_id ) ) {
		$class = esc_attr__( 'Liked','skyline' );
		$title = esc_attr__( 'Unlike','skyline' );
		$heart = '<i class="feather-heart"></i>';
	} else {
		$class = esc_attr( '' );
		$title = esc_attr__( 'Like','skyline' );
		$heart = '<i class="feather-heart"></i>';
	}
