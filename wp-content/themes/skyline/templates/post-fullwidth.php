...
?>
<div class='height-100 fullwidth-blog-img parallax' style='background-image:url(<?php echo esc_url($img_url[0]);?>)'>
...
&nbsp;&nbsp;&nbsp;<span class="blog-icons"><a href="javascript:window.print()"><i class="feather-printer"></i></a><a href="#comments"><i class="feather-speech-bubble"></i><?php comments_number( '0', '1', '%' ); ?></a><?php echo getPostLikeLink( $post->ID ); ?></span>
</div>
</div>
</div>
<?php
...
} elseif ($post_format_string == "Standard" || $post_format_string == "Gallery") { 
	// Title Removal for Standard Post Full Width
} else { ?>
	<header class="entry-header">
<?php the_title( '<h1 class="entry-title">', '</h1>' ); ?>
</header><!-- .entry-header -->
 <div class="entry-meta">
<?php echo get_avatar( get_the_author_meta( 'ID' ), 128 ); ?> by <?php the_author_posts_link(); ?> on <a href="<?php echo esc_url(get_day_link(get_post_time('Y'), get_post_time('m'), get_post_time('j'))); ?>"><?php the_time( get_option( 'date_format' ) ); ?></a>
