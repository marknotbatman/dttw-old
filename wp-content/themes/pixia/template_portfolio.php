...
?>
    <div id="content" class="<?php echo CONTAINER_CLASSES; ?> top_0">
        ...
        if ($my_query->have_posts()) : 
						$ins=0;
						echo '<div id="folio_classic" style="margin-top:'.$default_margin.'px;margin-right:-'.$default_margin.'px;" data-margin="'.$default_margin.'" class="'.$parent_class.'">';
						...
						<div id="post-<?php the_ID(); ?>" class="portfolio_entry_li <?php echo $skills_yo; ?> p_all" data-id="id-<?php echo $ins; ?>" style="margin-bottom:<?php echo $default_margin; ?>px;">
							...
							?>
						<a href="<?php echo $href_val; ?>"<?php if ($make_lbox=="yes" && $data['skip_to_external']==0){echo ' class="'.$magnifier_class.$vid_cl.'"';} ?> <?php if ($data['skip_to_external']==1){echo 'target="_blank"';}; ?>  data-title="<?php echo get_the_title(); ?>">
								<div class="grid_image_wrapper">
                                    ...
                                    if (has_post_thumbnail( $post->ID ) ) {
										...
										{
											?>
											<img src="<?php echo get_template_directory_uri(); ?>/inc/modules/timthumb/timthumb.php?src=<?php echo $image[0]; ?>&w=480&h=300&f=2" class="custom-img grid_image<?php if ($pixia_frontend_options['blog_bw']=="yes") echo " desaturated_image"; ?>" alt="" />
											<?php
										}
                                    }
                                    ?>
					...
					<?php 
						...
						if ($paged!=$my_query->max_num_pages)
						{
							...
							<div id="next_portfolio_grid" class="prk_next_wrapper">
