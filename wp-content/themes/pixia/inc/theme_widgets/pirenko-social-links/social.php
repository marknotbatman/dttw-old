	...
	*/
	...
	class pirenko_social_widget extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct( 'pirenko-social-widget', __('Pirenko: Social Links ', 'pirenko-social-widget'), $widget_ops, $control_ops );
		}
		...
		function widget( $args, $instance ) 
		{
			...
			<div id="pirenko_social" class="cf">
				...
				foreach ( $this->fields_array( $instance ) as $key => $data ) 
				{
					...
					{
						printf( '<a class="" href="%s" %s><img src="%s" pir_title="%s" data-pb-tint-opacity="1" class="pir_icons %s" /></a>', esc_url( $instance[$key] ), $new_window, esc_url(get_template_directory_uri().'/inc/theme_widgets/pirenko-social-links/icons/'.$instance['pir_icons'].'/'.$data['img'] ), esc_attr( $data['img_title'] ),$tips_class );
						$inside_counter++;
					}
				}
				?>
