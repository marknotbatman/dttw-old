		...
		//widget output
			public function widget($args, $instance) {
				...
				if ( ! empty( $title ) ){ echo $before_title . $title . $after_title; }
					...
					$pixia_twitter_options=get_option('pixia_theme_options');
					...
					if(!empty($tp_twitter_plugin_tweets)){
						...
						<div class="tp_recent_tweets">';
							...
							foreach($tp_twitter_plugin_tweets as $tweet){								
								print '<li><div class="icon-twitter"><i class=pixia_fa-twitter clearer_inactive_color"></i></div><div class="prk_twt_body"><span>'.convert_links($tweet['text']).'</span><div class="clearfix" style="margin-bottom:2px;"></div><a class="twitter_time" target="_blank" href="http://twitter.com/'.$instance['username'].'/statuses/'.$tweet['status_id'].'">'.relative_time($tweet['created_at']).'</a> - <a class="twitter_time" target="_blank" href="http://twitter.com/'.$instance['username'].'"">'.$instance['username'].'</a></div></li>';
