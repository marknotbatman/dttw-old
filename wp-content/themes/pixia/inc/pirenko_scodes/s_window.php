	...
	<div id="output_shortcode"></div>
        <div id="prk_codes_wrapper">
            ...
            if ($popup=="members")
            {
                ?>
                  	<tr>
                    	<td width="50%"><strong>Number of columns</strong><br /><em></em></td>
                    	<td>
                        <select id="sh_layout_type">
                      <?php 
                      $options=array(
                      '0' => array(
                        'value' =>  '2',
                ...
                ?>
                  	...
                  	<tr>
                    	<td width="50%"><strong>Members Group</strong><br /></td>
                    	<td>
							<?php
                                $terms=get_terms('pirenko_member_group');
                                $count = count($terms);
                                if ($count>0)
                                {
                                    ?>
                                    <select id="sh_slider_cat">
                                      <?php
                                          echo "\n\t<option style=\"padding-right: 10px;\" selected='selected' value=''>Show All</option>";
                                          $count=0; 
	...
	jQuery(document).ready(function() 
	{
		...
		{	 			
			if(window.tinyMCE)
			{
				...
				var sh_output="";
        if (shortcode_type=="members")
        {
          sh_output+='[prk_members columns="'+jQuery('#sh_layout_type').val()+'" category="'+jQuery('#sh_slider_cat').val()+'"][/prk_members]';
        }
        ...
        var prk_legacy = "<?php echo floatval(get_bloginfo('version')); ?>";
        if ( prk_legacy >= 3.9){
          window.tinyMCE.execCommand('mceInsertContent', false, jQuery('#output_shortcode').html());
        }
        else
        {
