...
<!--[if gt IE 8]><!--> <html class="no-js" <?php language_attributes(); ?>> <!--<![endif]--><head>
  ...
  <?php
	...
	function jquery_sender() 
	{
		...
		global $portfolio_link_ms;
		$blog_linked="";
		if ((is_single() && get_post_type()=="post") || (is_archive() && (!is_plugin_active('woocommerce/woocommerce.php') || (is_plugin_active('woocommerce/woocommerce.php') && !is_woocommerce()) )))
		{
	    ...
	    if (is_plugin_active('woocommerce/woocommerce.php')) {
	    	if (is_woocommerce() || is_cart() || is_checkout() || is_order_received_page()) {
	    		$woo_link=get_page_link(wc_get_page_id('shop'));
	    	}
	    }
		...
		<script type="text/javascript">
			...
			var woo_link="<?php echo $woo_link; ?>";
        ...
        if ($pixia_frontend_options['header_font']['hosted']=='google')
        {
            wp_enqueue_style( 'prk_header_font', "$protocol://fonts.googleapis.com/css?family=".$pixia_frontend_options['header_font']['value'] );
        }
        if ($pixia_frontend_options['header_font']['hosted']=='theme')
        {
            wp_enqueue_style( 'prk_header_font', get_template_directory_uri() . '/inc/fonts/'.$pixia_frontend_options['header_font']['value'].'/stylesheet.css',false,$prk_theme->Version);
        }
        if ($pixia_frontend_options['header_font']['hosted']=='plugin')
        {
            wp_enqueue_style( 'prk_header_font', $pixia_frontend_options['header_font']['value'],false,$prk_theme->Version);
        }
        if ($pixia_frontend_options['body_font']['hosted']=='google')
        {
            wp_enqueue_style( 'prk_body_font', "$protocol://fonts.googleapis.com/css?family=".$pixia_frontend_options['body_font']['value'] );
        }
        if ($pixia_frontend_options['body_font']['hosted']=='theme')
        {
            wp_enqueue_style( 'prk_body_font', get_template_directory_uri() . '/inc/fonts/'.$pixia_frontend_options['body_font']['value'].'/stylesheet.css',false,$prk_theme->Version);
        }
        if ($pixia_frontend_options['body_font']['hosted']=='plugin')
        {
            wp_enqueue_style( 'prk_body_font', $pixia_frontend_options['body_font']['value'],false,$prk_theme->Version);
        }
	?>
...
<body <?php body_class('pixia_theme '.roots_body_class()); ?>>
	 <?php
