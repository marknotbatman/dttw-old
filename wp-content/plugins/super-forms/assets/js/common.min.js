...
(function($) {
    ...
    // Validate the form
    SUPER.validate_form = function( $form, $submit_button, $validate_multipart ) {
    ...
    SUPER.google_places.initAutocomplete = function(){
        $('.super-form').find('.super-address-autopopulate:not(.super-autopopulate-init)').each(function(){
            var $element = $(this);
                ...
                if( $array.length>0 ) {
                    for (var $counter = 0; $counter < $array.length; $counter++) {
                        ...
                        if($element.length!=0){
                            ...
                            var $new_values = '';
                            var $new_value_array = $element.val().toString().split(',');
                            $.each($new_value_array, function( k, v ) {
                                var v = v.toString().split(';');
                                if($value_n==0){
                                    $new_value = v[0];
                            ...
                            if($value_n!=0){
                                $html = $html.replace('{field_'+$name+';'+$value_n+'}', $new_value).replace('{'+$name+';'+$value_n+'}', $new_value);
                            }else{
    ...
    SUPER.init_print_form = function($form){
            ...
            if($field.hasClass('super-dropdown')){
                ...
                if( (typeof $default_value !== 'undefined') && ($default_value!='') ) {
                    ...
                    if($field.find('.super-dropdown-ui > li.selected').length==0){
                        ...
                        $field.find('.super-placeholder').html($new_value);
                    }
                }
                return true;
            }
    ...
    SUPER.init_super_form_frontend = function(){
        ...
        SUPER.google_places.initAutocomplete();
    ...
    SUPER.init_field_filter_visibility = function() {
        $('.field-container.filter, .field.filter, .super-field.filter').each(function(){
            ...
            if(typeof $parent_field_name !== 'undefined'){
                ...
                if(typeof $parent_field.val()!=='undefined'){
    ...
    SUPER.super_find_next_tab_field = function($field, $form, $next_tab_index){
        ...
        if(typeof $field.attr('data-super-custom-tab-index') !== 'undefined'){
            var $next_tab_index = parseFloat($field.attr('data-super-custom-tab-index'));
        }
    ...
    jQuery(document).ready(function ($) {
        ...
        */
        ...
        $doc.on('keyup', '.super-form .super-text .super-distance-calculator', function(){
            var $this = $(this);
            var $form = $this.parents('.super-form:eq(0)');
            var $value = $this.data('distance-value');
            var $units = $this.data('distance-units');
            if($value!='dis_text'){
                var $units = 'metric';
            }
            var $method = $this.data('distance-method');
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        if($result.status=='OK'){
                            ...
                            if( ($value=='distance') || ($value=='dis_text') ){
                                if($value=='duration'){
                                    var $calculation_value = $leg.distance.value // in meters
                                }else{
                                    var $calculation_value = $leg.distance.text // in meters
                                }
                            }else{
                                if($value=='duration'){
                                    var $calculation_value = $leg.duration.value // in seconds
                                }else{
                                    var $calculation_value = $leg.duration.text // in seconds
                                }
                            }
                ...
                if($dropdown.length){
                    ...
                    if( ($element.length) && (!$element.hasClass('super-textarea') ) ) {
                        if(!$form.find('.super-form-button.super-loading').length){
                            SUPER.before_validating_form_hook(undefined, $form);
                            $submit_button = $form.find('.super-form-button > a');
                            SUPER.validate_form( $form, $submit_button );
