 <?php
 // FOR NAV TYPE 4 - KEEP SIDE MENU OPEN
if ($skyline_data['nav_type'] == "4") { ?>
<style type="text/css">
.mobile-menu-bar {
	display: none;
}
...
$('.header4 .mobile-menu-bar i.menu-bars-icon').click(function() {
	$('.header4').removeClass('nav-closed');
	$('.mobile-menu-bg').addClass('open');
	$('.mobile-menu-bar').addClass('fade');
});
$('.header4 a.close-icon').click(function() {
	$('.header4').addClass('nav-closed');
	$('.mobile-menu-bg').removeClass('open');
	$('.mobile-menu-bar').removeClass('fade');
